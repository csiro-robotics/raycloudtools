...
{
  ray::FileArgument cloud_file, mesh_file, trunks_file, trees_file;
  ...
  else if (extract_leaves)
  {
    ...
    ray::Cuboid grid_bounds = bounds;
    grid_bounds.min_bound_ -= Eigen::Vector3d(vox_width, vox_width, vox_width);
    ray::DensityGrid grid(grid_bounds, vox_width, dims);    
        ...
        for (int j = 0; j<subvoxels; j++)
        {
          for (int k = 0; k<subvoxels; k++)
          {
            while (leaves_added < leaves_per_subvoxel*count)
            {
              ...
              Leaf new_leaf;
              new_leaf.centre = Eigen::Vector3d(ray::random(minp[0], maxp[0]), ray::random(minp[1], maxp[1]), ray::random(minp[2], maxp[2]));
    ...
    leaf_verts.push_back(Eigen::Vector3d(len/2.0,len,0));
    leaf_verts.push_back(Eigen::Vector3d(0,2.0*len,-0.5*len));
    ...
    ray::writePlyMesh(cloud_file.nameStub() + "_leaves.ply", mesh);
  }
  else
  {
    usage(true);
