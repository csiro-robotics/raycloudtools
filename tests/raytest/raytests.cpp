...
namespace raytest
{
  ...
  {
    return raytest::Environment::Command(system_command);
  }
  ...
  std::size_t compareMoments(const Eigen::ArrayXd &m1, const std::vector<double> &m2, double eps = 0.1)
  {
    size_t i;
    for (i = 0; i<m2.size(); i++)
    {
      EXPECT_NEAR(m1[i], m2[i], eps);
  ...
  /// Compare the statistical (1st and 2nd order) moments of the two ray clouds. This almost surely
  /// detects differing clouds, and always equal clouds, given a tolerance @c eps.
  std::size_t compareMomentsPercentageError(const Eigen::ArrayXd &m1, const std::vector<double> &m2, double percentage = 5.0)
  {
    size_t i;
    for (i = 0; i<m2.size(); i++)
    {
      double eps = std::abs(m2[i]) * percentage * 0.01;
      EXPECT_NEAR(m1[i], m2[i], eps);
      if (std::abs(m1[i] - m2[i]) > eps)
      {
        return i;
  ...
  TEST(Basic, RayAlign)
  {
    ...
    EXPECT_EQ(compareMoments(cloud.getMoments(), {-0.0618268, -0.077552, 0.0531072, 7.58334e-08, 7.97642e-08, 1.93877e-08, -0.180532, -0.219257, 0.0654452, 2.47241, 2.08183, 1.28226, 17.539, 10.1994, 0.304682, 0.761892, 0.429502, 0.987362, 0.318932, 0.225742, 0.389901, 0.111705}), cloud.getMoments().size());
  }
  ...
  TEST(Basic, RayColour)
  {
    ...
    EXPECT_EQ(compareMoments(cloud.getMoments(), {-0.108066, -0.0410134, 0.052168, 7.05134e-08, 8.45038e-08, 1.93877e-08, -0.276144, -0.0760758, 0.065631, 2.42455, 2.13738, 1.28226, 17.539, 10.1994, 0.497919, 0.496369, 0.490293, 0.987362, 0.248361, 0.203648, 0.385192, 0.111705}), cloud.getMoments().size());
  }
  ...
  TEST(Basic, RayCombine)
  {
    ...
    EXPECT_EQ(compareMoments(cloud.getMoments(), {-0.0867714, -0.0679941, 0.546619, 0.0215326, 0.0272819, 0.499969, -0.305657, -0.186353, 0.582642, 2.95777, 2.47531, 1.63323, 17.4967, 10.1789, 0.305355, 0.763356, 0.427376, 0.979005, 0.318409, 0.225661, 0.389366, 0.143369}), cloud.getMoments().size());
  }
  ...
  TEST(Basic, RayCreate)
  {
    ...
    EXPECT_EQ(compareMoments(cloud.getMoments(), {-3.168, 16.5472, 7.04175, 3.28539, 18.0871, 2.96654, -3.19551, 16.564, 7.30826, 4.14359, 18.2463, 3.34431, 935.715, 540.236, 0.499997, 0.500408, 0.429416, 0.998894, 0.372134, 0.372389, 0.390499, 0.0332398}), cloud.getMoments().size());
  }
  ...
  TEST(Basic, RayDecimate)
  {
    ...
    EXPECT_EQ(compareMoments(cloud.getMoments(), {-0.222571, 1.08156, 1.67264, 6.00755, 5.78731, 0.508713, -0.202668, 1.09517, 2.6238, 6.0285, 5.85715, 3.22093, 69.0574, 35.2775, 0.48969, 0.498403, 0.443549, 1, 0.379062, 0.366963, 0.389535, 0}), cloud.getMoments().size());
  }
  ...
  TEST(Basic, RayRestore)
  {
    ...
    EXPECT_EQ(compareMoments(cloud.getMoments(), {2.07399, 0.575952, 3.05217, 7.85442e-08, 7.70963e-08, 1.93877e-08, 1.9391, 0.682169, 3.06563, 2.10068, 2.45642, 1.28226, 17.539, 10.1994, 0.304682, 0.761892, 0.429502, 0.987362, 0.318932, 0.225742, 0.389901, 0.111705}), cloud.getMoments().size());
  }
  ...
  TEST(Basic, RayRotate)
  {
    ...
    EXPECT_EQ(compareMoments(cloud.getMoments(), {-0.254879, 0.846076, 2.02322, 5.62648, 5.68622, 2.56306, 0.266873, 0.772016, 3.2888, 5.54595, 5.69021, 4.28394, 62.683, 36.1903, 0.514327, 0.504407, 0.413534, 1, 0.372377, 0.365965, 0.391709, 0}), cloud.getMoments().size());
  }
  ...
  TEST(Basic, RaySmooth)
  {
    ...
    EXPECT_EQ(compareMoments(cloud.getMoments(), {-0.108066, -0.0410134, 0.052168, 7.05134e-08, 8.45038e-08, 1.93877e-08, -0.27615, -0.0761079, 0.0656267, 2.42413, 2.13691, 1.28163, 17.539, 10.1994, 0.304682, 0.761892, 0.429502, 0.987362, 0.318932, 0.225742, 0.389901, 0.111705}), cloud.getMoments().size());
  }
  ...
  TEST(Basic, RaySplit)
  {
    ...
    EXPECT_EQ(compareMoments(cloud.getMoments(), {-0.467731, 1.05075, 1.43662, 2.20441, 1.60162, 0.106775, -0.77974, 1.03139, 1.57353, 3.67521, 2.64766, 0.485084, 17.3995, 10.279, 0.311066, 0.759795, 0.425206, 0.951355, 0.321609, 0.226785, 0.39073, 0.215125}), cloud.getMoments().size());
  }
  ...
  TEST(Basic, RayTransients)
  {
    ...
    EXPECT_EQ(compareMoments(cloud.getMoments(), {-1.05406, -0.240721, -0.0629182, 5.05649e-08, 3.32941e-08, 2.54759e-08, 0.268724, -0.136746, -0.596782, 1.04798, 0.921776, 0.527205, 32.1452, 6.7491, 0.205871, 0.395641, 0.884296, 1, 0.225501, 0.296487, 0.153923, 0}), cloud.getMoments().size());
  }
  ...
  TEST(Basic, RayTranslate)
  {
    ...
    ray::Cloud cloud;
    EXPECT_TRUE(cloud.load("forest.ply"));
    EXPECT_EQ(compareMoments(cloud.getMoments(), {9.66298, 21.3454, 31.7177, 6.0926, 5.75511, 0.56438, 9.69155, 21.3605, 33.0883, 6.10555, 5.82564, 3.20507, 62.683, 36.1903, 0.514327, 0.504407, 0.413534, 1, 0.372377, 0.365965, 0.391709, 0}), cloud.getMoments().size());
  }
  ...
  TEST(Basic, RayWrap)
  {
    ...
    EXPECT_EQ(compareMoments(mesh.getMoments(), {0.0386662, -1.52168, -0.139079, 3.30621, 3.35391, 0.705937}), mesh.getMoments().size());
  }
  ...
  TEST(Basic, RayExtract)
  {
    ...
    EXPECT_EQ(compareMoments(mesh.getMoments(), {-0.00147491, -0.00191917, -0.0617946, 5.77995, 5.80266, 0.0426993}), mesh.getMoments().size());
    EXPECT_EQ(command("rayextract trees forest.ply forest_mesh.ply"), 0);

    ray::ForestStructure forest;
    EXPECT_TRUE(forest.load("forest_trees.txt"));
    EXPECT_EQ(compareMomentsPercentageError(forest.getMoments(), {20, 22.2172, 1058.42, 1.39416, 0.112794, 2.2863, 21918, 0, 105.4}), forest.getMoments().size());
    ...
    EXPECT_EQ(compareMomentsPercentageError(forest2.getMoments(), {11, 8.43829, 586.427, 1.40054, 0.200644, 0, 16697, 8.49419, 3.09917}), forest2.getMoments().size());

    EXPECT_EQ(command("rayextract trunks forest.ply"), 0);

    ray::ForestStructure forest3;
    EXPECT_TRUE(forest3.load("forest_trunks.txt"));
    EXPECT_EQ(compareMomentsPercentageError(forest3.getMoments(), {21, 20.0797, 1124.61, 1.60427, 0.135159, 0, 0, 0, 0}), forest3.getMoments().size());
  }
