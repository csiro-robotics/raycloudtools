...
namespace ray
{
void Terrain::getParetoFrontFast(std::vector<Vector4d> points, std::vector<Vector4d> &front, bool positive)
{
  struct Node
  {
    Node()
...
void Terrain::getParetoFront(std::vector<Vector4d> points, std::vector<Vector4d> &front, bool pos)
{
  while (!points.empty())
  {
    Vector4d b = points.back();
    points.pop_back();
  
    for (int i = (int)points.size()-1; i>=0; i--)
    {
      if (greaterThan(b, points[i], pos))
      {
        points[i] = points.back();
        points.pop_back();
      }
      else if (greaterThan(points[i], b, pos))
      {
        b = points[i];
        points[i] = points.back();
        points.pop_back();
...
void Terrain::extract(const Cloud &cloud, const std::string &file_prefix, bool median, bool verbose)
{
  ...
  points.clear();
  double c = 0.5;
  points.push_back(Vector4d(3,0,0,c++));
  points.push_back(Vector4d(0,3,0,c++));
  points.push_back(Vector4d(0,0,3,c++));
  points.push_back(Vector4d(1,1,1,c++));
  points.push_back(Vector4d(0.1,0.1,0.1,c++));
  points.push_back(Vector4d(-3,0,0,c++));
  points.push_back(Vector4d(0,-3,0,c++));
  points.push_back(Vector4d(0,0,-3,c++));
  points.push_back(Vector4d(-1,-1,-1,c++));
...
//    getParetoFront(points_pos, pos2, true); 
  ...
  hull.growUpwards(0.01); // same as a Delauney triangulation
  hull.mesh().vertices() = vecs;
  writePlyMesh(file_prefix + "_median_mesh.ply", hull.mesh(), true);
  if (verbose)
  {
    Cloud local_cloud;
    double t = 0.0;
    for (auto &p: vecs)
      local_cloud.addRay(p, p, t++, white);
    local_cloud.save(file_prefix + "_median.ply");
  }
}
