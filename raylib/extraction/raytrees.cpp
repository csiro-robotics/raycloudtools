...
Trees::Trees(Cloud &cloud, const Mesh &mesh, const TreesParams &params, bool verbose)
{
  ...
  for (int i = (int)sections.size() - 1; i >= 0; i--)
  {
    ...
    //   std::cout << "initial end distances: " << sections[i].roots.size() << ": " << sections[i].max_distance_to_end
    //   << " rad: " << sections[i].radius << std::endl;
  }
  ...
  for (size_t i = 0; i < points.size(); i++) points[i].visited = false;
  ...
  for (size_t sec = 0; sec < sections.size(); sec++)
  {
    ...
    if (max_radius < 0.5 * params.min_diameter)
    {
      ...
      for (auto &i : sections[sec].ends) sections[sec].tip += points[i].pos;
      size_t sum = sections[sec].ends.size();
      if (sections[sec].ends.empty())
      {
        for (auto &i : sections[sec].roots) sections[sec].tip += points[i].pos;
    ...
    if (!extract_from_ends)
    {
      ...
      for (auto &root : sections[sec].roots) base += points[root].pos;
      ...
      std::vector<std::vector<int>> clusters =
        generateClusters(ps, params.gap_ratio * max_radius, params.span_ratio * max_radius, true);
      for (auto &cluster : clusters)
      {
        for (auto &id : cluster) id = v_indices[id];
      }
      size_t min_size = 1;
      if (par == -1)
      {
      ...
      {
        max_distances[i] = 0;
        for (auto &end : clusters[i]) max_distances[i] = std::max(max_distances[i], points[end].distance_to_end);
    ...
    for (auto &i : list) sections[sec].tip += points[i].pos;
    if (list.size() > 0)
...
#define REAL_CENTROID
      ...
      for (int ii = start_ii; ii < 2; ii++)
      {
        ...
        for (auto &i : node_list)  // one iteration of operation to find centre, using centroid, direction and radius
                                   // estimation as a prior guess
        {
    ...
    if (sections[sec].ends.size() > 0)
    {
      ...
      if (new_node.radius > 0.5 * params.min_diameter)  // if it is the first node, then we need a second noded
      {
        ...
        for (auto &end : new_node.roots) new_node.tip += points[end].pos;
  ...
  for (size_t sec = 0; sec < sections.size(); sec++)
  {
    ...
    if (sections[sec].children.size() == 0)  // then also add any offshoots...
    {
      ...
      {
        for (auto &c : children[ends[i]]) ends.push_back(c);
      }
    }
    for (auto &node : nodes) section_ids[node] = (int)sec;
    for (auto &end : ends) section_ids[end] = (int)sec;
  }
  for (size_t i = 0; i < points.size(); i++)
  {
    if (section_ids[i] == -1)  // an unfound point
    {
      ...
      while (j != -1 && section_ids[j] == -1) j = points[j].parent;
      if (j != -1)
