...
bool generateLeaves(const std::string &cloud_stub, const std::string &trees_file, const std::string &leaf_file,
                    double leaf_area, double droop, bool stalks)
{
  ...
  {  // Tim: this block looks for the closest cylindrical branch segments to each voxel, in order to give the leaves a
    ...
    const int search_size =
      12;  // find the twelve nearest branch segments. For larger voxels a larger value here would be helpful
  ...
  {
    leaf_counter[i] =
      (double)(::ray::rand() % 10000) / 10000.0;  // a random start stops regions of low density have 0 leaves
  }
