...
// Do I
// 1. seek the start and update the #vertices after every chunk (slow?)
// 2. only update the #vertices at the very end
// 
// 2 requires a separate writePlyChunkEnd, and file is broken if crashes midway
// 1 is cleaner but probably slower
// where am I keeping count of the total number?
// Instead, I could read the total ray size from the readPly function.. .
// ... but it wouldn't work well with the decimate function
                   ...
                   const std::vector<RGBA> &colours, bool /*start_chunk*/, bool end_chunk)
{
  ...
  if (out.tellp() == 0) // start_chunk) // not written to yet, so do the header stuff
  {
    out << "ply" << std::endl;
  ...
  unsigned long vec_size = sizeof(Eigen::Matrix<float, 9, 1>);
  out.write((const char *)&vertices[0], vec_size * vertices.size());   

  if (end_chunk) // go back to the start of the file and replace the vertex count with the correct value
  {
    unsigned long size = (unsigned long) out.tellp() - chunk_header_length;
    unsigned long number_of_rays = size / vec_size;
    out.seekp(vertex_size_pos);
    out << number_of_rays; // TODO 1: does this insert or replace the spaces?
                           // TODO 2: will 'out' run out of memory? do I need to actually save to file each chunk?
  }
}
             ...
             int chunk_size)
{
  ...
  for (size_t i = 0; i < size; i++)
  {
    ...
    if (ends.size() == chunk_size || i==size-1)
    {
      ...
      apply(starts, ends, times, colours, i==chunk_size-1, i==size-1);
      ends.clear();
      times.clear();
      colours.clear();
    }
