...
namespace 
{
unsigned long chunk_header_length = 0; // this is set once and is constant after that
unsigned long vertex_size_pos = 0;     // this is set once and is constant after that
}  
                        ...
                        const std::vector<double> &times, const std::vector<RGBA> &colours)
{
  ...
  out.write(reinterpret_cast<const char *>(&vertices[0]), sizeof(PointPlyEntry) * vertices.size());
  if (!out.good())
  {
    std::cerr << "Error: problem writing file " << file_name << std::endl;
    return false;
  }
  return true;
}
...
{
  std::ofstream ofs;
  if (!writePlyChunkStart(out_name, ofs))
    return false;
