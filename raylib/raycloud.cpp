...
void Cloud::getSurfels(int searchSize, vector<Vector3d> *centroids, vector<Vector3d> *normals,
                       vector<Vector3d> *dimensions, vector<Matrix3d> *mats, MatrixXi *neighbourIndices)
{
  ...
  for (unsigned int i = 0; i < rayIDs.size(); i++) pointsP.col(i) = ends[rayIDs[i]];
  ...
  for (int i = 0; i < (int)rayIDs.size(); i++)
  {
    ...
    {
      int j;
      for (j = 0; j < searchSize && indices(j, i) > -1; j++) (*neighbourIndices)(j, I) = rayIDs[indices(j, i)];
      if (j < searchSize)
    ...
    int num;
    for (num = 0; num < searchSize && indices(num, i) > -1; num++) centroid += ends[rayIDs[indices(num, i)]];
...
void Cloud::generateEllipsoids(vector<Ellipsoid> &ellipsoids)
{
  ...
  for (unsigned int i = 0; i < ends.size(); i++) pointsP.col(i) = ends[i];
  nns = Nabo::NNSearchD::createKDTreeLinearHeap(pointsP, 3);
...
void fillGrid(Grid<int> &grid, const vector<Vector3d> &starts, const vector<Vector3d> &ends)
{
  ...
  for (int i = 0; i < (int)ends.size(); i++)
  {
    ...
    Vector3i startIndex(start.cast<int>());
    Vector3i endIndex(end.cast<int>());
    ...
    for (;;)
    {
      ...
      if (d[0] < d[1] && d[0] < d[2])
        index[0] += int(dirSign[0]);
      else if (d[1] < d[0] && d[1] < d[2])
        index[1] += int(dirSign[1]);
      else
        index[2] += int(dirSign[2]);
    }
  }
...
void Cloud::markIntersectedEllipsoids(Grid<int> &grid, vector<bool> &transients, vector<Ellipsoid> &ellipsoids,
                                      const string &mergeType, double numRays, bool selfTransient)
{
  ...
  for (auto &ellipsoid : ellipsoids)
  {
    ...
    if (bMax[0] < 0.0 || bMax[1] < 0.0 || bMax[2] < 0.0)
      continue;
    if (bMin[0] > (double)grid.dims[0] - 1.0 || bMin[1] > (double)grid.dims[1] - 1.0 ||
        bMin[2] > (double)grid.dims[2] - 1.0)
      continue;
    Vector3i bmin = maxVector(Vector3i(0, 0, 0), Vector3i(bMin.cast<int>()));
    Vector3i bmax =
      minVector(Vector3i(bMax.cast<int>()), Vector3i(grid.dims[0] - 1, grid.dims[1] - 1, grid.dims[2] - 1));
    ...
    for (auto &rayID : rayIDs)
    {
      ...
      bool passThrough =
        rayLength * (1.0 - ratio) > d + alongDist;  // last number requires rays to pass some way past the object
      if (passThrough)
        passThroughIDs.push_back(rayID);
      else
      {
        hits++;
        firstIntersectionTime = min(firstIntersectionTime, times[rayID]);
        lastIntersectionTime = max(lastIntersectionTime, times[rayID]);
      }
    }
    size_t numBefore = 0, numAfter = 0;
    ...
    if (selfTransient)
    {
      ...
      double misses = 0;
      for (auto &rayID : passThroughIDs)
      {
        if (times[rayID] > lastIntersectionTime)
          numAfter++;
        else if (times[rayID] < firstIntersectionTime)
          numBefore++;
        else
          misses++;
      }
    ...
    else  // compare to other cloud
    {
      if (passThroughIDs.size() > 0)
      {
        if (times[passThroughIDs[0]] > ellipsoid.time)
          numAfter = passThroughIDs.size();
        else
          numBefore = passThroughIDs.size();
      }
    }
    ...
    if (type == 0 || type == 1)
    {
      if (double(std::max(numBefore, numAfter)) < sequenceLength)
        continue;
      if (type == 0)                                    // oldest
        removeEllipsoid = double(numBefore) >= sequenceLength;  // if false then remove numAfter rays if > seqLength
      else if (type == 1)                               // newest
        removeEllipsoid = double(numAfter) >= sequenceLength;   // if false then remove numBefore rays if > seqLength
    }
    else
    {
      ...
      if (double(numBefore + numAfter) < sequenceLength)
        continue;
