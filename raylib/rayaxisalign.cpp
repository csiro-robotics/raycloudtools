...
// Apply the radon transform to the 2D array representing density of end points (and their centroid), and convert the peak into a 2D pose
// For convenience we use the 3D Euclidean transformation class (Pose), and set the vertical translation to the mid point.
Pose estimate2DPose(const Eigen::Array<Eigen::Vector3d, Eigen::Dynamic, Eigen::Dynamic> &position_accumulator, const Cloud::Info &info)
{
...
bool alignCloudToAxes(const std::string &cloud_name, const std::string &aligned_file)
{
  ...
  double peak_k = static_cast<double>(max_k) + peak(vertical_weights[std::max(0, max_k-1)], vertical_weights[max_k], vertical_weights[std::min(max_k+1, amp_res-1)]);
