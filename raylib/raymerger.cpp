...
void markIntersectedEllipsoids(const ray::Cloud &cloud, ray::Grid<int> &grid, std::vector<bool> &transients,
                               std::vector<ray::Ellipsoid> &ellipsoids, const std::string &merge_type, double num_rays,
                               bool self_transient)
{
  ...
  for (auto &ellipsoid : ellipsoids)
  {
    ...
    Eigen::Vector3i bmax = ray::minVector(Eigen::Vector3i(b_max.cast<int>()),
                                          Eigen::Vector3i(grid.dims[0] - 1, grid.dims[1] - 1, grid.dims[2] - 1));
...
void ray::Merger::mergeSingleCloud(const ray::Cloud &cloud, const std::string &merge_type, double num_rays,
                                   bool colour_cloud)
{
  const double voxel_width = 4.0 * cloud.estimatePointSpacing();
  std::cout << "find transients" << std::endl;

  std::vector<ray::Ellipsoid> ellipsoids;
  Eigen::Vector3d bounds_min;
  Eigen::Vector3d bounds_max;
  generateEllipsoids(&ellipsoids, &bounds_min, &bounds_max, cloud);

  ray::Grid<int> grid(bounds_min, bounds_max, voxel_width);
  fillGrid(grid, cloud);
...
void ray::Merger::mergeMultipleClouds(std::vector<ray::Cloud> &clouds, const std::string &merge_type, double num_rays)
{
  ...
  for (int c = 0; c < (int)clouds.size(); c++)
  {
    ...
    generateEllipsoids(&ellipsoids, nullptr, nullptr, clouds[c]);
    // just set opacity
...
void ray::Merger::mergeThreeWay(const ray::Cloud &base_cloud, ray::Cloud &cloud1, ray::Cloud &cloud2,
                                const std::string &merge_type, double num_rays)
{
  ...
  std::cout << clouds[0]->ends.size() << " and " << clouds[1]->ends.size() << " rays to combine, that are different"
            << std::endl;
