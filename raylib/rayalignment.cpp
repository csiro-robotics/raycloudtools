...
static const double highPassPower = 0.25; // This fixes inout->inout11, inoutD->inoutB2 and house_inside->house3. Doesn't break any. power=0.25. 0 is turned off. 
...
void Array3D::fillWithRays(const Cloud &cloud)
{
  ...
  for (int i = 0; i<(int)cloud.ends.size(); i++)
  {
    ...
    while ((index - startIndex).squaredNorm()<=lengthSqr+1e-10)
    {
      ...
      if (d[0] < d[1] && d[0]<d[2])
        index[0] += dirSign.cast<int>()[0];
      else if (d[1] < d[0] && d[1] < d[2])
        index[1] += dirSign.cast<int>()[1];
      else
        index[2] += dirSign.cast<int>()[2];
    }
  }
}
